...
def get_random_video_id(channel):
	url = "https://api.yogsdb.com/api/videos?channel="+ channel +"&orderBy=upload_date|desc"
	result = parse_json(url)
	if not result['data']:
		return "Could not find channel with ID " + channel
	random_page = randint(1, int(result['last_page']))
	url = "https://api.yogsdb.com/api/videos?channel="+ channel +"&orderBy=upload_date|desc&page=" + str(random_page)
	result = parse_json(url)
	selected_video = choice(result['data'])
